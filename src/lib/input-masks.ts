...
export function isBrazilianDocument(value: string) {
  const digits = normalizeBrazilianDocument(value);
  return digits.length === 11 || digits.length === 14;
}

export function getBrazilianDocumentType(value: string): "CPF" | "CNPJ" {
  return normalizeBrazilianDocument(value).length === 14 ? "CNPJ" : "CPF";
}

export function isBrazilianDocumentType(value: string, type: "CPF" | "CNPJ") {
  const digits = normalizeBrazilianDocument(value);
