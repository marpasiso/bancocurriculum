...
export async function confirmPendingPixPaymentAction(formData: FormData) {
  const admin = await requireOperationalAdminUser();
  try {
    await confirmAdminPendingPixPayment(admin.id, {
      employerId: getString(formData, "employerId"),
      paymentId: getString(formData, "paymentId")
    });
  } catch (error) {
...
export async function confirmCandidateHiringByAdminAction(formData: FormData) {
  const admin = await requireOperationalAdminUser();
  try {
    await markManagedCandidateAsHired(admin.id, admin.role, {
      candidateId: getString(formData, "candidateId")
    });
  } catch (error) {
    redirect(`/admin/candidatos?error=${errorParam(toFriendlyError(error))}`);
...
export async function returnCandidateToAvailableByAdminAction(formData: FormData) {
  const admin = await requireOperationalAdminUser();
  try {
    await returnManagedCandidateToAvailable(admin.id, admin.role, {
      candidateId: getString(formData, "candidateId")
    });
  } catch (error) {
    redirect(`/admin/candidatos?error=${errorParam(toFriendlyError(error))}`);
  ...
  redirect("/admin/candidatos?candidateAvailable=1");
}
