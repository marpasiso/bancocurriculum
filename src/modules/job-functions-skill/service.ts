...
export function normalizeSystemJobFunctionCategory(category?: string) {
  return category?.trim().replace(/\s+/g, " ") || "Geral";
}
...
export async function saveSystemJobFunction(actorUserId: string, input: unknown) {
  ...
  const data = {
    ...
    category: normalizeSystemJobFunctionCategory(parsed.category)
  };
  ...
  await recordAuditLog({
    ...
    metadata: { fields: ["name", "category", "description", "isActive"] }
  });
...
export async function createGlobalJobFunction(actorUserId: string, input: unknown) {
  ...
  await recordAuditLog({
    ...
    entity: "SystemJobFunction",
    entityId: jobFunction.id,
    metadata: { name: jobFunction.name, category: jobFunction.category }
  });
