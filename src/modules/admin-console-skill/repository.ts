...
export async function updateCandidateActiveStatus(candidateId: string, isActive: boolean) {
  return prisma.candidate.update({
    ...
    select: { id: true, fullName: true, email: true, isActive: true, availabilityStatus: true }
  });
}

export async function updateCandidateAvailabilityStatus(input: {
  candidateId: string;
  availabilityStatus: "AVAILABLE" | "IN_PROCESS" | "HIRED" | "UNAVAILABLE";
  isActive?: boolean;
}) {
  return prisma.candidate.update({
    where: { id: input.candidateId },
