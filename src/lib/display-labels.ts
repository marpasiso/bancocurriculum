import type { LgpdRequestType, PaymentStatus, UserRole } from "@prisma/client";
...
const lgpdRequestTypeLabels: Record<LgpdRequestType, string> = {
  ...
  REVOCATION: "Revogação da autorização"
};

const userRoleLabels: Partial<Record<UserRole, string>> = {
  ADMIN: "Administrador",
  EMPLOYER: "Empregador",
...
export function formatUserRole(role: UserRole) {
  return userRoleLabels[role] ?? "Usuário administrativo";
}
